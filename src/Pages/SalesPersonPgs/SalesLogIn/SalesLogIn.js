import styles from './SalesLogIn.module.css';
...
const SalesLogIn = ({ setLoggedIn }) => {
    ...
    const handleLogin = async (e) => {
        ...
        try {
            ...
            const response = await fetch(`${url_}/authenticate/salesmanger`, requestOptions);
            ...
            if (response.status === 200) {
                ...
                const salesmanager_id = result.user.id;
                const salesmanager_name = result.user.name;
                const salesmanager_mobile = result.user.mobile;
                const salesmanager_pan = result.user.pan;
                const salesmanager_email = result.user.email;



                localStorage.setItem('jwtToken', jwtToken);
                localStorage.setItem('salesmanager_name', salesmanager_name);
                localStorage.setItem('salesmanager_id', salesmanager_id);
                localStorage.setItem('LogedIn', 'true');
                localStorage.setItem('mobile', salesmanager_mobile);
                localStorage.setItem('pan', salesmanager_pan);
                localStorage.setItem('email', salesmanager_email);
