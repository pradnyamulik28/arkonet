...
const SubUserLogin = ({ setLoggedIn }) => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        function CurrentTimeConverter(LoginDate) {
        ...
        const user_id = result.sub_user.userid;
        const user_pan = result.sub_user.userPan;

        const Sub_user_name = result.sub_user.name;
        const Sub_user_mobile = result.sub_user.mobile;
        const Sub_user_email = result.sub_user.email;
        const Sub_user_id = result.sub_user.id;
        const Sub_user_pan = result.sub_user.pan;
        const End_Date = result.sub_user.endDate;
        const CurrentTimeNow = CurrentTimeConverter(new Date());

        localStorage.setItem('jwtToken', jwtToken);
        localStorage.setItem('user_id', user_id);
        localStorage.setItem('pan', user_pan);

        localStorage.setItem('name', Sub_user_name);
        localStorage.setItem('Sub_user_id', Sub_user_id);
        localStorage.setItem('LogedIn', 'true');
        localStorage.setItem('mobile', Sub_user_mobile);
