...
const CheckPassSetPass = () => {
  ...
  const handleSetPassword = async () => {
    ...
    if (userData.password !== userData.cpassword) {
      await Swal.fire("Failed!!", "Password did not match!!!", "error")
    } else {
      ...
      try {
        ...
        const response = await fetch(`${url_}/SubUser_SetPassword?otp=${localStorage.getItem('otp')}&newPassword=${userData.password}`, requestOptions)
        ...
        if (response.status === 200) {
          ...
          await Swal.fire("Failed!!", "Failed to set password. Please try again!!!", "error")
          setuserData({
            password: "",
            cpassword: ""
          })
              ...
              {newsetPassword ? (
                ...
                <div className={styles.form}>
                  ...
                  <div className={styles.user_id}>
                    <InputField placeholder='Enter your password...' onChange={handleChange} name='password' value={userData.password} lblname='Password' type="password" />
                  </div>
                  <div className={styles.user_pass}>
                    <InputField placeholder='Re-enter your password...' onChange={handleChange} name='cpassword' value={userData.cpassword} lblname='Confirm Password' type="password" />
                  </div>
...
export default CheckPassSetPass;
