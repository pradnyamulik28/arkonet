...
const CheckPan = ({ setLoggedIn }) => {
  ...
  const handlePanCheck = async () => {
    ...
    try {
      ...
      // myHeaders.append("Authorization", `Bearer eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiJQQVZBTjE5OTlKIiwiaWF0IjoxNzA0Mjk4OTE0LCJleHAiOjE3MDQzNDIxMTR9.lMLF58i659pssT5RdHIRW3izRHwpgNjUFLJaA6K889c`);
      ...
      const response = await fetch(`${url_}/SubUsers/isPasswordNull/${userPan.pan}`, requestOptions)
      const result = await response.json()
      // console.log(response.status)
      // console.log(result.Category === "User")
      // console.log(result.isPasswordNull === true)
      ...
      if (response.status === 200) {
        ...
        if (result.Category === "User") {
          localStorage.setItem("User_Pan", userPan.pan)
          localStorage.setItem("Category", result.Category)
          SendOTP();


        } else {
          if (result.isPasswordNull === true) {
            localStorage.setItem("User_Pan", userPan.pan)
            localStorage.setItem("Category", result.Category)
            SendOTP();
          } else {
        ...
        await Swal.fire("Failed!!", `${result.message}!!!`, "error")
  ...
  const SendOTP = async () => {
    ...
    try {
      ...
      const response = await fetch(`${url_}/send-otp_ToSubUser?pan=${userPan.pan}`, requestOptions);
      const result = await response.text();
      console.log(result);

      // Close the loading spinner after the API response is received
      // Swal.close();
      ...
      if (response.status === 200) {
        ...
        await Swal.fire("Failed!!", "Failed to send OTP. Please try again!!!", "error");
      }
    } catch (error) {
      console.log(error);
      // Close loading spinner in case of an error
      // Swal.close();
    }
  };
            ...
            <div className={styles.main}>
              ...
              <div className={styles.form}>
                ...
                <div className={styles.btn_login}>
                  <button onClick={handlePanCheck}>NEXT</button>
                </div>
