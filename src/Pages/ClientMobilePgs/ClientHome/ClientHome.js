...
function ClientHome() {
  ...
  async function getGstFilestatus() {
    ...
    if (month < 3) {
      await fetch(`${url_}/GST_Statusfilednotfiled/${user_id_gst}/${client_id_gst}/${year - 1}/GSTR-1`, requestOptions)
        .then(response => response.text())
        .then(result => {//console.log(result)
    ...
    if (month < 3) {
      await fetch(`${url_}/GST_Statusfilednotfiled/${user_id_gst}/${client_id_gst}/${year - 1}/GSTR-3B`, requestOptions)
        .then(response => response.text())
        .then(result => {//console.log(result)
        ...
        <div className={`${style.data}`}>
          ...
          {client_id_gst && <div className={`${style.gstdata}`}>
            <div className={`${style.gsthead}`}>
              <p>&bull;</p>
              <h5 className={gstSubStatus === "off" ? `${style.disabled_link}` : `${style.h51}`} onClick={(e) => {
                e.preventDefault();
