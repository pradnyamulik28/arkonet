...
const DistributorLogIn = ({ setLoggedIn }) => {
    ...
    const handleLogin = async (e) => {
        ...
        try {
            ...
            const response = await fetch(`${url_}/authenticate/Distribution`, requestOptions);
            ...
            if (response.status === 200) {
                ...
                const distributor_id = result.user.id;
                const distributor_name = result.user.name;
                const distributor_mobile = result.user.mobile;
                const distributor_pan = result.user.pan;
                const distributor_email = result.user.email;



                localStorage.setItem('jwtToken', jwtToken);
                localStorage.setItem('distributor_name', distributor_name);
                localStorage.setItem('distributor_id', distributor_id);
                localStorage.setItem('LogedIn', 'true');
                localStorage.setItem('mobile', distributor_mobile);
                localStorage.setItem('pan', distributor_pan);
                localStorage.setItem('email', distributor_email);


                // console.log(result)
                Navigate('distributor_dash');
                setLoggedIn(true);
          ...
          <div className={styles.right_body}>
            ...
            <div className={styles.main}>
              ...
              <div className={styles.link}>
                <Link to="distributor_reg">New to TAXKO? Click Here</Link>
